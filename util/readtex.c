...
static void 
RawImageGetData(rawImageRec * raw, RGBImage * final)
{
  unsigned char *ptr;
  int i, j;
...
LoadRGB(const char *imageFile)
{
  ...
  {
      image->format = GL_RGB;
//		fprintf(stderr, "Format GL_RGB\n");
  } else if (image->components == 4)
  {
      image->format = GL_RGBA;
//		fprintf(stderr, "Format GL_RGBA\n");
  } else if (image->components == 1)
  {
      image->format = GL_LUMINANCE;
//		fprintf(stderr, "Format LUMINANCE\n");
  }
